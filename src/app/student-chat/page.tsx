...
const initialMessages: {[key: string]: Message[]} = {
    counselor1: [
        { role: 'professional', content: "Hello! How can I help you today?" },
    ],
    peer1: [
         { role: 'professional', content: "Hey, feel free to reach out if you need to talk." },
    ],
};
...
function StudentChatContent() {
  ...
  const [selectedConversation, setSelectedConversation] = useState(
    professionals.find(p => p.id === professionalIdFromParams) || professionals[0]
  );
  const [messages, setMessages] = useState<Message[]>(initialMessages[selectedConversation.id] || []);
  const [input, setInput] = useState('');
  const [isSending, setIsSending] = useState(false);

  const handleSelectConversation = (professional: typeof professionals[0]) => {
      setSelectedConversation(professional);
      setMessages(initialMessages[professional.id] || []);
  }

  const handleSendMessage = () => {
    if (input.trim() === '') return;
    setIsSending(true);
    const newMessage: Message = { role: 'student', content: input };
